 ...
 * Keep in sync with backend/utils/recipeImageHeuristic.js
 */
(function (global) {
    'use strict';
    ...
    function normalizeRecipeText(r) {
        const tags = Array.isArray(r && r.tags) ? r.tags.join(' ') : '';
        const ing = ingredientsToText(r && r.ingredients);
        const desc = (r && r.description && String(r.description).slice(0, 200)) || '';
        return `${(r && r.title) || ''} ${(r && r.category) || ''} ${(r && r.cuisine) || ''} ${tags} ${ing} ${desc}`.toLowerCase();
    }

    function getHeuristicRecipeImageUrl(r) {
        const t = normalizeRecipeText(r);
        const seed = (r && r.title) || t;
        ...
        for (let i = 0; i < matchers.length; i++) {
            ...
            for (let j = 0; j < m.keys.length; j++) {
                if (t.indexOf(m.keys[j]) !== -1) return pickStable(pools[m.pool], seed);
            }
        }
        ...
        if (cat.indexOf('lunch') !== -1) return pickStable(pools.salad, seed);
        if (cat.indexOf('dinner') !== -1) return pickStable(pools.generic, seed);
        return pickStable(pools.generic, seed);
    }

    global.getHeuristicRecipeImageUrl = getHeuristicRecipeImageUrl;
    global.normalizeRecipeTextForImage = normalizeRecipeText;
